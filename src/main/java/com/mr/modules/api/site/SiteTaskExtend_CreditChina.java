...
public class SiteTaskExtend_CreditChina extends SiteTaskExtend {
	...
	protected boolean saveAdminPunishOne(AdminPunish adminPunish, Boolean isForce) {
		...
		//生成UniqueKey
		if (StrUtil.isEmpty(adminPunish.getUniqueKey())) {
			String bizKey = adminPunish.getUrl() + "@"
					+ adminPunish.getEnterpriseName() + "@"
					+ adminPunish.getPersonName() + "@"
					+ adminPunish.getJudgeNo() + "@"
					+ adminPunish.getJudgeAuth() + "@";

			adminPunish.setUniqueKey(bizKey);
		}
	...
	protected boolean saveDisneycreditBlackListOne(DiscreditBlacklist discreditBlacklist, Boolean isForce) {
		...
		//生成UniqueKey
		if (StrUtil.isEmpty(discreditBlacklist.getUniqueKey())) {
			String bizKey = discreditBlacklist.getUrl() + "@"
			...
			discreditBlacklist.setUniqueKey(bizKey);
		}
		try {
			...
			writeBizErrorLog(discreditBlacklist.getUrl(), e.getMessage());
		}
		return isFlag;
	}
