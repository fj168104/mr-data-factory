...
@Component("guizhou")
@Scope("prototype")
public class SiteTaskImpl_BOIS_GuiZhou  extends SiteTaskExtend{
    @Override
    protected String execute() throws Throwable {
//        String url = "http://guizhou.circ.gov.cn/web/site17/tab3467/module9099/page1.htm";
    ...
    public List extractPageUrlList(){
        ...
        //第一个页面，用于获取总页数
        String baseUrl = "http://guizhou.circ.gov.cn/web/site17/tab3467/module9099/page1.htm";
        //解析第一个页面，获取这个页面上下文
        String fullTxt = getData(baseUrl);
        //获取页数
        int  pageAll= extractPage(fullTxt);
        for(int i=1;i<=pageAll;i++){
            String url ="http://guizhou.circ.gov.cn/web/site17/tab3467/module9099/page"+i+".htm";
    ...
    public List<Map<String,String>> extractContent(String fullTxt) {
        ...
        //发布机构
        String publishOrg = "中国保监会贵州保监局行政处";
        //发布时间
        String publishDate = "";
        //TODO 处罚机关（由于有些页面没有，所以暂且给予默认值）
        String punishOrg = "贵州保监局";
        //TODO 处罚时间
        String punishDate = "";
        //TODO 处罚文号
        String punishNo = "";
        ...
        if((txtAll.contains("当事人：")&&!txtAll.contains("违法违规行为")&&!txtAll.contains("备注"))||txtAll.contains("遵义市邮政局")||txtAll.contains("遵义县邮政局")){
            ...
            for(String arrStr : txtAllArr){
                ...
                if(arrStr.contains("时任")&&!arrStr.contains("时任人")){
                    priJob.append(arrStr.substring(arrStr.indexOf("时任")+2,arrStr.length())).append("，");
                }
                if(arrStr.contains("时任人")){
                    priJob.append(arrStr.substring(arrStr.indexOf("时任人")+3,arrStr.length())).append("，");
                }
            }
            if(punishOrg.equals("")){
                punishOrg ="黔保监局";
        ...
        }else if(!txtAll.contains("当事人：")&&txtAll.contains("：经查")){
            for(String arrStr : txtAllArr) {
                String[] str = arrStr.split("：");
            ...
            stringDetail =elementsSpan.text();
            if(stringDetail.contains("存在")&&stringDetail.contains("经查")&&!titleStr.contains("保监罚")&&titleStr.contains("）")&&titleStr.contains("（")){
                String[] titleNames = titleStr.replace(",","，").replace("（","（，").replace("）","，）").split("（")[1].split("）")[0].split("，");
                if(titleNames.length>1){
                    for(String titleName : titleNames){
                        if(!titleName.equals("")){
                            if(titleName.trim().length()<6){
                                //TODO 受处罚当时人名称（自然人）
                                priPerson.append(titleName).append("，");
                                //TODO 判断处罚的是法人，还是自然人
                                personFlag=true;
                            }else{
                                //TODO 受处罚机构
                                punishToOrg.append(titleName).append("，");
                                //TODO 判断处罚的是法人，还是自然人
                                personFlag=false;
                            }
                        }
