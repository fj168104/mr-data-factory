...
@Component("chongqing")
@Scope("prototype")
public class SiteTaskImpl_BOIS_HeNan extends SiteTaskExtend{
    /**
     * 河南保监局处罚 提取所需要的信息
    ...
    public Map extractContent(String fullTxt) {
			...
			for(Element elmtTD:elementsTD){
				...
				if(elementsP.size()>0){
					...
					if(s.indexOf("、")>0 || s.indexOf(",")>0){
						priPerson=s.indexOf("、")>0?s.substring(s.indexOf("、")+1):
							s.indexOf("，")>0?s.substring(s.indexOf("，")+1):"";
						if(priPerson.length()>0 && priPerson.indexOf("）")>0)
		...
		html=html.replaceAll("　", "")
				...
				.replaceAll("受处罚人：姓名：", "受处罚人：")
				.replaceAll("受处罚人姓名：", "受处罚人：")
				.replaceAll("，地址", "\n地址:")
				.replaceAll("，负责人", "\n主要负责人姓名:")
				.replaceAll("年龄", "\n年龄")
				.replaceAll("性别", "\n性别")
				.replaceAll("\n姓名：", "\n主要负责人姓名:")
				.replaceAll("负责人：", "主要负责人姓名:")
		...
		for(String content:contents){
			...
			if(punishToOrg.length()<1 && 
				...
				if(content.indexOf("公司")>0){
					...
					}else if(content.indexOf("事业部")>0){
						punishToOrg=content.substring(0,content.indexOf("事业部")+3);
					}else if(content.indexOf("营业部")>0){
						punishToOrg=content.substring(0,content.indexOf("营业部")+3);
...
//					}
				...
				if(punishToOrg.indexOf("任")>=0 && punishToOrg.indexOf("责任")<0){
...
//					punishToOrgAddress="";
			...
			if(index<contents.length-2 && contents[index+1].indexOf("年龄")>=0){
				String tmpStr=content;
				if(tmpStr.indexOf(":")>0){
					priPerson=priPerson.length()>0 ? priPerson+"，"+tmpStr.substring(tmpStr.indexOf(":")+1).trim():tmpStr.substring(tmpStr.indexOf(":")+1).trim();
				}else{
					priPerson=priPerson.length()>0 ? priPerson+"，"+tmpStr.trim():tmpStr.trim();
				}
			}
			...
			if(priJob.length()<1 && (content.indexOf("职务:")>=0 || content.indexOf("任")>0 || content.indexOf("经理")>0|| content.indexOf("助理")>0 || content.indexOf("主任")>0 || content.indexOf("董事")>0)){
				//System.out.println("----------------");
				if(content.indexOf("经理")>0 || (content.indexOf("任")>0 && content.indexOf("责任")<0 && content.indexOf("任用")<0 && content.indexOf("的")<0)){
					if(content.indexOf("经理")>0){
						priJob=content.substring(0,content.indexOf("经理")+2);
					}
					if(content.indexOf("任")>0)
						priJob=content.substring(content.indexOf("任")+1);
					if(content.indexOf("对")>=0)
						priJob=content.substring(content.indexOf("对")+1);
		...
		System.out.println("--------------------------------------------------");
		System.out.println("发布主题：" + pubTitle);
		System.out.println("发布机构：" + publishOrg);
		System.out.println("发布时间：" + publishDate);
		System.out.println("处罚机关：" + pubOffice);
		System.out.println("处罚时间：" + punishDate);
		System.out.println("处罚文号：" + punishNo);
		System.out.println("受处罚机构：" + punishToOrg);
		System.out.println("受处罚机构地址：" + punishToOrgAddress);
		System.out.println("受处罚机构负责人：" + punishToOrgHolder);
		System.out.println("受处罚人：" + priPerson);
		System.out.println("受处罚人证件：" + priPersonCert);
		System.out.println("受处罚人职位：" + priJob);
		System.out.println("受处罚人地址：" + priAddress);
		System.out.println("--------------------------------------------------");
		System.out.println("受处文本：" + punishTxtAll);		
...
protected String execute() throws Throwable {
    String url = "http://www.circ.gov.cn/web/site0/tab5241/info185700.htm";
    String fullTxt = getData(url);
    extractContent(fullTxt);
    log.info("----------------------河南保监局处罚信息提取完成------------------------");
    return null;
}
}	
