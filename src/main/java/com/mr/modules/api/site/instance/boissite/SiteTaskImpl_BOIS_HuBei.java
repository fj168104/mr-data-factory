...
@Component("hubei")
@Scope("prototype")
public class SiteTaskImpl_BOIS_HuBei extends SiteTaskExtendSub {
    @Override
    protected String execute() throws Throwable {
//        String url = "http://hubei.circ.gov.cn/web/site21/tab3434/module8998/page1.htm";
    ...
    public List extractPageUrlList(){
        ...
        //第一个页面，用于获取总页数
        String baseUrl = "http://hubei.circ.gov.cn/web/site21/tab3434/module8998/page1.htm";
        //解析第一个页面，获取这个页面上下文
        String fullTxt = getData(baseUrl);
        //获取页数
        int  pageAll= extractPage(fullTxt);
        for(int i=1;i<=pageAll;i++){
            String url ="http://hubei.circ.gov.cn/web/site21/tab3434/module8998/page"+i+".htm";
            ...
            for(Element element : elementsHerf){
                ...
                String resultUrl = "http://hubei.circ.gov.cn"+elementUrl.attr("href");
    ...
    public Map extractContent(String fullTxt) {
        //发布机构
        String publishOrg = "中国保监会湖北保监局行政处";
        //发布时间
        String publishDate = "";
        //TODO 处罚机关（由于有些页面没有，所以暂且给予默认值）
        String punishOrg = "湖北保监局";
        //TODO 处罚时间
        String punishDate = "";
        //TODO 处罚文号
        String punishNo = "";
        ...
        if((punishNo.equals("")||punishNo.equals("null")||punishNo.equals("NULL")||punishNo==null)&&titleStr.contains("保监罚")&&titleStr.endsWith("号")&&titleStr.contains("（")&&titleStr.endsWith("）")){
            //TODO 处罚文号
            if(titleStr.split("（").length>1){
                punishNo=titleStr.split("（")[1].split("）")[0];
            }
        }
        if((punishNo.equals("")||punishNo.equals("null")||punishNo.equals("NULL")||punishNo==null)&&titleStr.contains("处罚决定")&&titleStr.contains("号")){
            //TODO 处罚文号
            punishNo=titleStr.substring(titleStr.indexOf("处罚决定"),titleStr.indexOf("号")+1);
        }
