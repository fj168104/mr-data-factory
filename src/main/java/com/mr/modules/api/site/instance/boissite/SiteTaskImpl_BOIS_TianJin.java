...
@Scope("prototype")
@Slf4j
public class SiteTaskImpl_BOIS_TianJin  extends SiteTaskExtendSub {
    /**
     * 获取：全量、增量
...
//        String url = "http://tianjin.circ.gov.cn/web/site35/tab3385/module8892/page1.htm";
        ...
        for(String urlResult : urlList){
            ...
            Map map = extractContent(htmlPage.asXml());
            try{
    ...
    public List extractPageUrlList() throws Exception{
        List<String> urlList = new ArrayList<>();
        //第一个页面，用于获取总页数
        String baseUrl = "http://tianjin.circ.gov.cn/web/site35/tab3385/module8892/page1.htm";
        //解析第一个页面，获取这个页面上下文
        WebClient wb = new WebClient();
        wb.getOptions().setJavaScriptEnabled(false);
        wb.getOptions().setCssEnabled(false);
        HtmlPage htmlPage = wb.getPage(baseUrl);
        String fullTxt = htmlPage.asXml();

        //获取页数
        int  pageAll= extractPage(fullTxt);
        ok:for(int i=1;i<=pageAll;i++){
            String url ="http://tianjin.circ.gov.cn/web/site35/tab3385/module8892/page"+i+".htm";
            WebClient wbSub = new WebClient();
            wbSub.getOptions().setJavaScriptEnabled(false);
            wbSub.getOptions().setCssEnabled(false);
            HtmlPage htmlPageSub = wbSub.getPage(url);
            String resultTxt = htmlPageSub.asXml();
            ...
            for(Element element : elementsHerf){
                ...
                String resultUrl = "http://tianjin.circ.gov.cn"+elementUrl.attr("href");
    ...
    public List extractPageUrlListAdd(String date)throws Throwable{
        ...
        int  pageAll= extractPage(fullTxt);
        ok:for(int i=1;i<=pageAll;i++){
            String url ="http://tianjin.circ.gov.cn/web/site35/tab3385/module889/page"+i+".htm";
            WebClient wbSub = new WebClient();
            wbSub.getOptions().setJavaScriptEnabled(false);
            wbSub.getOptions().setCssEnabled(false);
            HtmlPage htmlPageSub = wbSub.getPage(url);
    ...
    public Map extractContent(String fullTxt) {
        //发布机构
        String publishOrg = "中国保监会天津保监局行政处";
        //发布时间
        String publishDate = "";
        //TODO 处罚机关（由于有些页面没有，所以暂且给予默认值）
        String punishOrg ="天津保监局";
        //TODO 处罚时间
        String punishDate = "";
        //TODO 处罚文号
        String  punishNo = "";
        ...
        //System.out.println("时间："+publishDate+"正文："+elementsTxt.text());
        //全文提取
        ...
        //System.out.println("txtAll:"+txtAll);
        ...
        if(txtAll.contains("当事人：")){
            for(String arrStr : txtAllArr){
                ...
                if(arrStr.contains("津")&&arrStr.contains("号")&&arrStr.contains("保监")){
                    //TODO 处罚文号
                    punishNo=arrStr.replaceAll(".*津","津").replaceAll("号.*","号");
                }
        ...
        if(punishNo.equals("")||punishNo.equals("null")||punishNo.equals("NULL")||punishNo==null){
            ...
            punishNo =titleStr.replaceAll(".*津","津").replaceAll("号.*","号");
            if(punishNo.equals("")||punishNo.equals("null")||punishNo.equals("NULL")||punishNo==null){
    ...
    public FinanceMonitorPunish getObj(Map<String,String> mapInfo, String href){
        ...
        financeMonitorPunish.setPartyInstitution(delFinallyString(mapInfo.get("punishToOrg"),"，").replace("（"," ").replace("）"," ").replaceAll("以下.*","").replaceAll("简称.*",""));//当事人（公司）=处罚对象
