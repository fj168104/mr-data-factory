...
//        String url = "http://anhui.circ.gov.cn/web/site11/tab3388/module8940/page1.htm";
    ...
    public List extractPageUrlList(){
        ...
        //第一个页面，用于获取总页数
        String baseUrl = "http://xizang.circ.gov.cn/web/site49/tab4613/";
        //解析第一个页面，获取这个页面上下文
        String fullTxt = getData(baseUrl);
        //获取页数
        int  pageAll= extractPage(fullTxt);
        for(int i=1;i<=pageAll;i++){
            String url ="http://xizang.circ.gov.cn/web/site49/tab4613/"+i+".htm";
            ...
            for(Element element : elementsHerf){
                ...
                String resultUrl = "http://xizang.circ.gov.cn"+elementUrl.attr("href");
    ...
    public Map extractContent(String fullTxt) {
        //发布机构
        String publishOrg = "中国保监会西藏保监局行政处";
        //发布时间
        String publishDate = "";
        //TODO 处罚机关（由于有些页面没有，所以暂且给予默认值）
        String punishOrg ="西藏保监局";
        //TODO 处罚时间
        String punishDate = "";
        //TODO 处罚文号
        String  punishNo = "";
        ...
        Map resMap = new ParseXiZang().parseInfo(fullTxt);
