    ...
    public  Map parseInfo(String fullText){
        ...
        if(stringDetail.indexOf("当事人")>-1){
            ...
            if(punishDate.equalsIgnoreCase("")){
                if(elementsPre.last()!=null && elementsPre.last().text().endsWith("日") && elementsPre.last().text().contains("年")){
            ...
            if(punishNo.equalsIgnoreCase("")){
                ...
                }else if(titleStr.contains("号") && titleStr.contains("罚")){
            ...
            if(punishDate.length()>12 ||punishDate.equalsIgnoreCase("")){
                if(elementsSpan.text().contains("年") && elementsSpan.text().contains("月") && elementsSpan.text().contains("日")
                        && elementsSpan.text().lastIndexOf("年") < elementsSpan.text().lastIndexOf("月") && elementsSpan.text().lastIndexOf("月") < elementsSpan.text().lastIndexOf("日")){
                    punishDate = formatText(elementsSpan.text().substring(elementsSpan.text().lastIndexOf("年")-4,elementsSpan.text().lastIndexOf("日")+1));
                }
            }
            //TODO 需要判断是法人还是自然人
            ...
            for(int i=0;i<listStr.size();i++ ){
                ...
                if(textInfo.contains("当事人：") && textInfo.contains("地址：") && textInfo.contains("负责人：")){//此为法人
                    ...
                    if(currentPersonStr[1].length()>5&&currentPersonStr[0].trim().equals("当事人") && !currentPersonStr[1].contains("曾用名")){
                        m = i;
                        busiPersonFlag =true;
        ...
        resmap.put("stringDetail",textTransfer(elementsSpan.text().trim()).replaceAll("&&",""));
