...
public class ParseZheJiang {
    ...
    public  Map parseInfo(String fullText){
        ...
        if(punishDate.equalsIgnoreCase("")){
            if(elementsPre.last()!=null && elementsPre.last().text().indexOf("年")>-1 && elementsPre.last().text().indexOf("月")>-1
                    && elementsPre.last().text().indexOf("日")>-1){
        ...
        if(punishDate.equalsIgnoreCase("")){
            ...
            if(spanText.lastIndexOf("日")>spanText.lastIndexOf("月") && spanText.lastIndexOf("月") > spanText.lastIndexOf("年")){
                punishDate = getPunishDate(spanText);
            }
        }
        //如果正文里没有处罚文号
        ...
        if(stringDetail.indexOf("当事人")>-1){
            ...
            String info = stringDetail;
            if(info.contains("号") && info.contains("保监") && info.indexOf("保监")<info.indexOf("号")){
                info = info.substring(info.indexOf("号")+1).trim();
            }
            if(info.contains("：")){
                String name = info.split("：")[0];
                if(name.length()>5 && !name.contains("男") &&
                        !name.contains("女") && !name.contains("出生")){
                    punishToOrg = name;
                }else{
