 ...
 */
@Slf4j
@Component("creditchina-gansu-shixin-199795")
@Scope("prototype")
public class CreditChinaGansuShiXin199795 extends SiteTaskExtend_CreditChina {
	private String url = CreditChinaSite.GANSU.getBaseUrl() + "/shiXin/199795.jhtml";
	...
	public void extractContent(String url) throws Throwable {
		...
		trFor: for (Element tr : trs) {
			...
			blackList.setDiscreditType("2016年10月逾期3期及以上失信人员名单");
			...
			for (Element td : tds) {
				...
				String text = CrawlerUtil.replaceHtmlNbsp(td.text()).replace("　", " ").replace(" ", "");// 替换&nbsp;和空格
				if ("序号".equals(text)) {
					continue trFor;// 直接跳到下一行
				}
				tdNo++;
				switch (tdNo) {
					case 1:// 序号
						break;
					case 2:// 借款合同编号
						discreditAction = discreditAction + "借款合同编号：" + text + "，";
						break;
					case 3:// 借款人姓名
	...
	private DiscreditBlacklist createDefaultDiscreditBlacklist() {
		...
		blackList.setPublishDate("2016/11/23");// 发布日期
