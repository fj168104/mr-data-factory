 ...
 */
@Slf4j
@Component("creditchina-gansu-xzcf-gr")
@Scope("prototype")
public class CreditChinaGansuXzcfGR extends SiteTaskExtend_CreditChina {
	private String baseUrl = CreditChinaSite.GANSU.getBaseUrl();

	@Override
	protected String execute() throws Throwable {
		log.info("抓取“信用中国（甘肃）-行政处罚（自然人）”信息开始...");
		...
		log.info("抓取“信用中国（甘肃）-行政处罚（自然人）”信息结束！");
		return null;
	}
	...
	private HashSet<String> extractPageUrlList(String keyWord) {
		...
		try {
			// 解析第一个页面，获取这个页面上下文
			String indexHtml = getData(wc, baseUrl + "/sgs/xzcf/list_1.jspx?type=person&keyword=" + keyWord);
			// 获取总页数
			...
			for (int i = 1; i <= pageAll; i++) {
				...
				if (i == 1) {// 直接解析首页
					...
					String resultHtml = getData(wc, baseUrl + "/sgs/xzcf/list_" + i + ".jspx?type=person&keyword=" + keyWord);
	...
	public void extractContent(String url) throws Throwable {
		...
		if (li != null && li.size() > 0) {
			...
			adminPunish.setSubject("行政处罚（个人）");// 主题
			adminPunish.setUrl(url);// url
			adminPunish.setObjectType("02");// 主体类型: 01-企业 02-个人
