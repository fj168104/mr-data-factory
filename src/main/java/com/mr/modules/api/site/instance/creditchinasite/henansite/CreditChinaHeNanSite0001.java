...
/**
 * 河南省地税局重大税收违法案件	文本	主体名称 主体代码
 * url:http://www.xyhn.gov.cn/ca/20151124000001.htm
 */
@Slf4j
@Component("creditchinahenansite0001")
@Scope("prototype")
 public class CreditChinaHeNanSite0001 extends SiteTaskExtend_CreditChina{
    @Autowired
             ...
             if(map.size()>0){
                 ...
                 if(totalPage>1){
                     for(int i=2;i<=totalPage;i++){
                         urlMain = "http://www.xyhn.gov.cn/CMSInterface/cms/getMdList?id=BBC3EE15E95B4887B1BCAC69A4E4C75F&pagesize=20&start="+i+"&type=2&content="+ URLEncoder.encode(keyWord,"UTF-8");
    ...
    public DiscreditBlacklist discreditBlacklistInsert(Map<String,String> map){
        ...
        //subject	主题
        discreditBlacklist.setSubject("河南省地税局重大税收违法案件");
        //url	url
