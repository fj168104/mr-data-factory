...
/**
 * 涉金融领域失信关联黑名单	文本	主体名称 主体代码
 */

@Slf4j
@Component("creditchinahenansite0005")
@Scope("prototype")
public class CreditChinaHeNanSite0005 extends SiteTaskExtend_CreditChina {
    @Autowired
            ...
            if (map.size() > 0) {
                ...
                if (totalPage > 1) {
                    for (int i = 2; i <= totalPage; i++) {
                        urlMain = "http://www.xyhn.gov.cn/CMSInterface/cms/getMdList?id=a6be338bcd9411e78ce51c3947289257&pagesize=20&start=" + i + "&type=2&content=" + URLEncoder.encode(keyWord, "UTF-8");
    ...
    public DiscreditBlacklist discreditBlacklistInsert(Map<String, String> map) {
        ...
        //subject	主题
        discreditBlacklist.setSubject("涉金融领域失信关联黑名单");
        //url	url
