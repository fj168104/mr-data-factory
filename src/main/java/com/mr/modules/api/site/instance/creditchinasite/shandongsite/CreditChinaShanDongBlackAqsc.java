 ...
 * 信用中国（山东）-安全生产黑名单
 * 
 * http://www.creditsd.gov.cn/creditsearch.redlist.dhtml?source_id=158&type=black
 * 
 * @author pxu 2018年6月11日
 */
@Slf4j
@Component("creditchina-shandong-black-aqsc")
@Scope("prototype")
public class CreditChinaShanDongBlackAqsc extends SiteTaskExtend_CreditChina {
	@Override
	protected String execute() throws Throwable {
		log.info("抓取“信用中国（山东）-安全生产黑名单”信息开始...");
		...
		log.info("抓取“信用中国（山东）-安全生产黑名单”信息结束！");
		return null;
	}
	...
	private HashSet<String> extractPageUrlList(String keyWord) {
		...
		// 解析第一个页面，获取这个页面上下文
		String indexHtml = getData(baseUrl + "/creditsearch.redlist.dhtml?source_id=158&kw=" + keyWord + "&page=1");
		// 获取总页数
		...
		for (int i = 1; i <= pageAll; i++) {
			...
			if (i == 1) {// 直接解析首页
				...
				String resultHtml = getData(baseUrl + "/creditsearch.redlist.dhtml?source_id=158&kw=" + keyWord + "&page=" + i);
	...
	public void extractContent(String url) throws Throwable {
		...
		blackList.setSubject("安全生产黑名单");// 主题
		...
		for (int i = 0; i < trs.size(); i++) {
			...
			if (Objects.equals(name, "单位名称")) {
				blackList.setEnterpriseName(value);// 企业名称
			} else if (Objects.equals(name, "注册地址")) {

			} else if (Objects.equals(name, "统一社会信用代码")) {
				String[] str = value.replace("组织机构代码：", "@!@组织机构代码：").replace("注册号：", "@!@注册号：").replace("工商注册号：", "@!@注册号：").split("@!@");
				for (int j = 0; j < str.length; j++) {
					if (str[j].trim().length() == 0) {
						continue;
					}
					if (str[j].contains("组织机构代码：")) {
						blackList.setEnterpriseCode3(str[j].replace("组织机构代码：", "").trim());// 组织机构代码
						continue;
					}
					if (str[j].contains("注册号：")) {
						blackList.setEnterpriseCode2(str[j].replace("注册号：", "").trim());// 营业执照注册号
						continue;
					}
					blackList.setEnterpriseCode1(str[j]);// 统一社会信用代码
				}
			} else if (Objects.equals(name, "主要负责人")) {
				blackList.setPersonName(value);// 法定代表人/负责人姓名|负责人姓名
			} else if (Objects.equals(name, "失信行为简况")) {
				blackList.setDiscreditAction(value);// 失信行为
			} else if (Objects.equals(name, "信息报送机关")) {
				blackList.setJudgeAuth(value);// 判决机关
			} else if (Objects.equals(name, "纳入理由")) {
				blackList.setPunishReason(value);// 列入原因
			} else if (Objects.equals(name, "备注")) {
