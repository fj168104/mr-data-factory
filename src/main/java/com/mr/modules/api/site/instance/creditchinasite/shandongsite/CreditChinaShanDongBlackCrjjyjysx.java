...
/**
 * 信用中国（山东）-2016年度出入境检验检疫严重失信企业名单
 * 
 * http://www.creditsd.gov.cn/creditsearch.redlist.dhtml?source_id=161&type=black
 * 
 * @author pxu 2018年6月11日
 */
@Slf4j
@Component("creditchina-shandong-black-crjjyjysx")
@Scope("prototype")
public class CreditChinaShanDongBlackCrjjyjysx extends SiteTaskExtend_CreditChina {
	@Override
	protected String execute() throws Throwable {
		log.info("抓取“信用中国（山东）-2016年度出入境检验检疫严重失信企业名单”信息开始...");
		...
		log.info("抓取“信用中国（山东）-2016年度出入境检验检疫严重失信企业名单”信息结束！");
		return null;
	}
	...
	private HashSet<String> extractPageUrlList(String keyWord) {
		...
		// 解析第一个页面，获取这个页面上下文
		String indexHtml = getData(baseUrl + "/creditsearch.redlist.dhtml?source_id=161&kw=" + keyWord + "&page=1");
		// 获取总页数
		...
		for (int i = 1; i <= pageAll; i++) {
			...
			if (i == 1) {// 直接解析首页
				...
				String resultHtml = getData(baseUrl + "/creditsearch.redlist.dhtml?source_id=161&kw=" + keyWord + "&page=" + i);
	...
	public void extractContent(String url) throws Throwable {
		...
		blackList.setSubject("2016年度出入境检验检疫严重失信企业名单");// 主题
		...
		for (int i = 0; i < trs.size(); i++) {
			...
			if (Objects.equals(name, "检疫检疫验证失信企业名称")) {
				blackList.setEnterpriseName(value);// 企业名称
			} else if (Objects.equals(name, "企业组织机构代码")) {
