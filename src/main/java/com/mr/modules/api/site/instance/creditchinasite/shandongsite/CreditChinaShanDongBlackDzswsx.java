...
/**
 * 信用中国（山东）-电子商务领域严重失信企业黑名单
 * 
 * http://www.creditsd.gov.cn/creditsearch.redlist.dhtml?source_id=305&type=black
 * 
 * @author pxu 2018年6月11日
 */
@Slf4j
@Component("creditchina-shandong-black-dzswsx")
@Scope("prototype")
public class CreditChinaShanDongBlackDzswsx extends SiteTaskExtend_CreditChina {
	@Override
	protected String execute() throws Throwable {
		log.info("抓取“信用中国（山东）-电子商务领域严重失信企业黑名单”信息开始...");
		...
		log.info("抓取“信用中国（山东）-电子商务领域严重失信企业黑名单”信息结束！");
		return null;
	}
	...
	private HashSet<String> extractPageUrlList(String keyWord) {
		...
		// 解析第一个页面，获取这个页面上下文
		String indexHtml = getData(baseUrl + "/creditsearch.redlist.dhtml?source_id=305&kw=" + keyWord + "&page=1");
		// 获取总页数
		...
		for (int i = 1; i <= pageAll; i++) {
			...
			if (i == 1) {// 直接解析首页
				...
				String resultHtml = getData(baseUrl + "/creditsearch.redlist.dhtml?source_id=305&kw=" + keyWord + "&page=" + i);
	...
	public void extractContent(String url) throws Throwable {
		...
		blackList.setSubject("电子商务领域严重失信企业黑名单");// 主题
		...
		for (int i = 0; i < trs.size(); i++) {
			...
			} else if (Objects.equals(name, "失信领域")) {
				blackList.setDiscreditType(value);// 失信类型
			} else if (Objects.equals(name, "更新时间")) {
