 ...
 * 信用中国（山东）-欠税公告
 * 
 * http://www.creditsd.gov.cn/creditsearch.redlist.dhtml?source_id=4&type=black
 * 
 * @author pxu 2018年6月11日
 */
@Slf4j
@Component("creditchina-shandong-black-qsgg")
@Scope("prototype")
public class CreditChinaShanDongBlackQsgg extends SiteTaskExtend_CreditChina {
	@Override
	protected String execute() throws Throwable {
		log.info("抓取“信用中国（山东）-欠税公告”信息开始...");
	...
	private HashSet<String> extractPageUrlList(String keyWord) {
		...
		// 解析第一个页面，获取这个页面上下文
		String indexHtml = getData(baseUrl + "/creditsearch.redlist.dhtml?source_id=4&kw=" + keyWord + "&page=1");
		// 获取总页数
		...
		for (int i = 1; i <= pageAll; i++) {
			...
			if (i == 1) {// 直接解析首页
				...
				String resultHtml = getData(baseUrl + "/creditsearch.redlist.dhtml?source_id=4&kw=" + keyWord + "&page=" + i);
	...
	public void extractContent(String url) throws Throwable {
		...
		for (int i = 0; i < trs.size(); i++) {
			...
			if (Objects.equals(name, "纳税人识别号")) {
				sb.append(" 纳税人识别号：").append(value);
			} else if (Objects.equals(name, "纳税人名称")) {
				blackList.setEnterpriseName(value);// 企业名称
			} else if (Objects.equals(name, "法定代表人")) {
				blackList.setPersonName(value);// 法定代表人/负责人姓名|负责人姓名
			} else if (Objects.equals(name, "经营地点")) {
				sb.append(" 经营地点：").append(value);
			} else if (Objects.equals(name, "欠税金额（元）")) {
				sb.append(" 欠税金额（元）：").append(value);
			}
		}
		blackList.setPunishReason(sb.toString());// 列入原因
