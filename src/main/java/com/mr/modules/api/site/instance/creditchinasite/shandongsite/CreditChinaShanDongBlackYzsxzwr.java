 ...
 * 信用中国（山东）-严重失信债务人黑名单
 * 
 * http://www.creditsd.gov.cn/creditsearch.redlist.dhtml?source_id=315&type=black
 * 
 * @author pxu 2018年6月11日
 */
@Slf4j
@Component("creditchina-shandong-black-yzsxzwr")
@Scope("prototype")
public class CreditChinaShanDongBlackYzsxzwr extends SiteTaskExtend_CreditChina {
	@Override
	protected String execute() throws Throwable {
		log.info("抓取“信用中国（山东）-严重失信债务人黑名单”信息开始...");
		...
		log.info("抓取“信用中国（山东）-严重失信债务人黑名单”信息结束！");
		return null;
	}
	...
	private HashSet<String> extractPageUrlList(String keyWord) {
		...
		// 解析第一个页面，获取这个页面上下文
		String indexHtml = getData(baseUrl + "/creditsearch.redlist.dhtml?source_id=315&kw=" + keyWord + "&page=1");
		// 获取总页数
		...
		for (int i = 1; i <= pageAll; i++) {
			...
			if (i == 1) {// 直接解析首页
				...
				String resultHtml = getData(baseUrl + "/creditsearch.redlist.dhtml?source_id=315&kw=" + keyWord + "&page=" + i);
	...
	public void extractContent(String url) throws Throwable {
		...
		blackList.setSubject("严重失信债务人黑名单");// 主题
