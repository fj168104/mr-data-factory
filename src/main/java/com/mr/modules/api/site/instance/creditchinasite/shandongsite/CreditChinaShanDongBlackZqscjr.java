...
public class CreditChinaShanDongBlackZqscjr extends SiteTaskExtend_CreditChina {
	...
	private HashSet<String> extractPageUrlList() {
		...
		// 解析第一个页面，获取这个页面上下文
		String indexHtml = getData(baseUrl + "/creditsearch.redlist.dhtml?source_id=160&kw=&page=1");
		// 获取总页数
		...
		for (int i = 1; i <= pageAll; i++) {
			...
			if (i == 1) {// 直接解析首页
				...
				String resultHtml = getData(baseUrl + "/creditsearch.redlist.dhtml?source_id=160&kw=&page=" + i);
