...
public class CreditChinaShanDongNationTaxPunish extends SiteTaskExtend_CreditChina {
	...
	private HashSet<String> extractPageUrlList(String keyWord) {
		...
		// 解析第一个页面，获取这个页面上下文
		String indexHtml = getData(baseUrl + "/creditsearch.redlist.dhtml?source_id=31&kw=" + keyWord + "&page=1");
		// 获取总页数
		...
		for (int i = 1; i <= pageAll; i++) {
			...
			if (i == 1) {// 直接解析首页
				...
				String resultHtml = getData(baseUrl + "/creditsearch.redlist.dhtml?source_id=31&kw=" + keyWord + "&page=" + i);
	...
	public void extractContent(String url) throws Throwable {
		...
		blackList.setSubject("重大税收违法案件信息公告(省国税局)");// 主题
		...
		if (blackList.getEnterpriseName().equals(blackList.getPersonName())) {// 若企业名称=法人名称，则为自然人
