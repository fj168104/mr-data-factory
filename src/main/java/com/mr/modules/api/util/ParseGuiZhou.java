...
public class ParseGuiZhou {
    ...
    public  Map parseInfo(String fullText){
        ...
        }else if(stringDetail.contains("经查，") && stringDetail.contains("违反了")){
            for(Element elementP : elementsP){
                String elementPStr =  elementP.text().replaceAll("\\s*", "");
                elementPStr = textTransfer(elementPStr);
                if(elementPStr.startsWith("经查，") && elementPStr.contains("存在")){
                    String punishInfo = elementPStr.substring(elementPStr.indexOf("经查，")+3, elementPStr.indexOf("存在"));
                    if(punishInfo.length()>5){
                        punishToOrg = punishInfo;
                    }else{
                        priPerson = priPerson.append(priPerson).append("，");
                    }
                }
            }
